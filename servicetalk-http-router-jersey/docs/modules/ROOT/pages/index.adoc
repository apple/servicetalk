...
aggregation performed by the ServiceTalk transport:
...
un-aggregated. Entity readers that buffer it in memory -- JAX-RS's own `String`,
`byte[]` and form readers, and the `Buffer`, `Single<Buffer>` and single-object
JSON/Protobuf readers -- do that buffering inside Jersey, where the limit never
applies, so a large request body is buffered in memory in full.
...
`413 Payload Too Large` or by tearing down the connection. Make the filter the single
place the limit is expressed by disabling `maxAggregatedPayloadSize`, so there is only
...
CAUTION: The filter counts bytes where it sits in the chain, so one appended before a
content-decoding filter bounds the encoded body and leaves whatever it expands to
unbounded. Append it after decoding to bound what is held in memory, or append one
instance on each side to reject oversized wire bodies early and still bound the
expansion. See
