...
NOTE: The server's `maxAggregatedPayloadSize` limit applies to readers that
aggregate the whole body in memory -- `Buffer`, `Single<Buffer>`, and Jersey's
built-in `String`, `byte[]`, and form (`MultivaluedMap`/`Form`) readers -- which
respond `413 Payload Too Large` when exceeded. Genuinely streaming readers
(`Publisher<Buffer>`, and raw `InputStream`/`Reader`) are not bounded. See
xref:{page-version}@servicetalk-http-api::programming-paradigms.adoc[the payload
aggregation note].
