...
public final class ExecutionStrategyResources {
    ...
    public abstract static class AbstractExecutionStrategyResource {
        ...
        private Single<Map<String, String>> getThreadingInfoSingleMapped(final Single<Buffer> content) {
            ...
            return content.flatMap($ -> fromSameExecutorAs(resourceMethodThread) ? success(threadingInfo) :
                    error(new IllegalStateException("Expected resource method thread: " + resourceMethodThread +
                            ", but got: " + currentThread())));
        }
        ...
        private Publisher<HttpPayloadChunk> getThreadingInfoPublisherMapped(final Publisher<HttpPayloadChunk> content) {
            ...
            return content
                    .doOnRequest($ -> requireFromSameExecutorAs(resourceMethodThread))
                    .first()
                    .flatMapPublisher($ -> bufferSingle.flatMapPublisher(buf -> Publisher.from(newPayloadChunk(buf))));
        }
