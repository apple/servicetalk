...
import static io.servicetalk.http.router.jersey.AbstractExecutionStrategyTest.TestMode.GET_CHUNKED;
...
public abstract class AbstractExecutionStrategyTest extends AbstractJerseyHttpServiceTest {
    ...
    protected enum TestMode {
        GET, GET_CHUNKED, POST, POST_CHUNKED
    }
    ...
    static {
        ...
        SUB_SUB_PATH_TEST_MODES.put("-single-buffer", GET_CHUNKED);
    ...
    public void testResource() {
        ...
        switch (testMode) {
            case GET:
                sendAndAssertResponse(get(path), OK, APPLICATION_JSON, expectedExecInfo, String::length);
                break;
            case GET_CHUNKED:
                sendAndAssertResponse(get(path), OK, APPLICATION_JSON, expectedExecInfo, $ -> null);
                break;
            case POST:
    ...
    private Matcher<String> getExecutorMatcher(final ExpectedExecutor expectedExecutor) {
        switch (expectedExecutor) {
            case SERVER:
                return hasRunOn(startsWith("st-server-"), getServerExecutor());
            case ROUTER:
    ...
    protected abstract Executor getServerExecutor();
