...
ServiceTalk uses link:https://www.jacoco.org/jacoco[JaCoCo] for code coverage. Coverage is not wired into
`./gradlew build` to keep the default build fast; it is generated on demand.
...
The aggregate report configuration lives in `gradle/jacoco.gradle`; the per-module setup is applied by the
ServiceTalk Gradle plugin.
