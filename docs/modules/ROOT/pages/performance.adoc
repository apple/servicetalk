...
When Java allocates direct memory or an array it performs zero initialization for allocated region, according to the
link:https://docs.oracle.com/javase/specs/jls/se8/html/jls-4.html#jls-4.12.5[JLS, Initial Values of Variables]. This is
good for memory safety when users have access to the whole memory region. However, in ServiceTalk the allocated memory
is wrapped with link:{source-root}/servicetalk-buffer-api/src/main/java/io/servicetalk/buffer/api/Buffer.java[Buffer]
API that maintains read and write indexes to prevent access to unfilled regions. Therefore, zero initialization is not
necessary but introduces CPU overhead and affects the total throughput.

Zero initialization is disabled for default
...
when JDK8 is used. Users of JDK9+ will need to provide additional system properties to get the advantage of this
optimization.

To disable zero initialization for direct buffers in JDK9+, use:
...
To disable zero initialization for heap buffers in JDK9+, use:
...
[NOTE]
These flags do not disable zero initialization when the memory is allocated directly via `new byte[]`,
`ByteBuffer.allocate(int)`, or `ByteBuffer.allocateDirect(int)`, they optimize only behavior of ServiceTalk
