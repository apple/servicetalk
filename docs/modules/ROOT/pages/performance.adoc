...
Depending upon the use case, performance costs of offloading maybe relatively high but we have the following
compensatory strategies in place:
...
Offloading is the execution of program logic code using a thread (or equivalent) other than the application
...
execution requires offloading – the initiating application thread is not available. The use of protective offloading,
where offloading is used for avoid resource monopolization, is a practical consideration, but just as necessary for
...
Netty implements more performant JNI link:https://netty.io/wiki/native-transports.html[native transports]. Using those
instead of Java NIO will allow users to achieve better performance numbers, reduce latencies, and make application more
reactive to the network events. To verify that the native transport is available use
`io.netty.channel.epoll.Epoll.isAvailable()` or `io.netty.channel.kqueue.KQueue.isAvailable()`.

Users of the latest Linux kernels may experiment with link:https://unixism.net/loti/what_is_io_uring.html[io_uring], an
asynchronous I/O API through
link:https://github.com/netty/netty-incubator-transport-io_uring[netty-incubator-transport-io_uring] module. Read
documentation for the netty module to understand all requirements, limitations, and production-readiness.
Availability of this feature can be checked by `io.netty.incubator.channel.uring.IOUring`. If it's available, use
`-Dio.servicetalk.transport.netty.tryIoUring=true` system property to opt-in for io_uring transport instead of epoll.
...
The latest `netty-tcnative-boringssl-static` version can be found in
link:https://search.maven.org/artifact/io.netty/netty-tcnative-boringssl-static[Maven Central Repository].
Alternatively, rather than setting `$tcnativeVersion` to a specific version, you can omit the version and instead import the latest `netty-bom:$nettyVersion` from
link:https://search.maven.org/artifact/io.netty/netty-bom[Maven Central Repository]. The Netty BOM will specify the version for tcnative components appropriate for the Netty version you are using.
