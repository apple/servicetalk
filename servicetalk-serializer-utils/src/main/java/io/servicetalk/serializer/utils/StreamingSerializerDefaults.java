...
final class StreamingSerializerDefaults {
    ...
    // The maximum length declared by a frame's length prefix that a 2-arg serializer accepts by default. A value
    // <= 0 disables the limit; every other value is honored verbatim so the property can raise, lower, or disable it.
    ...
    static {
        ...
        if (value != DEFAULT_MAX_MESSAGE_SIZE_VALUE) {
            LOGGER.warn("-D{}: {}. This property is temporary and will be removed in a future release. Configure the " +
                            "limit per serializer via the 3-arg FixedLengthStreamingSerializer / " +
