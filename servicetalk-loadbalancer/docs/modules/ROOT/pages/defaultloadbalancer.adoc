...
All of the xDS outlier detectors are bounded by `maxEjectionPercentage`: an ejection only happens if it leaves the
ejected share of hosts at or below that percentage. Small host sets therefore see no xDS ejection at all, which keeps
a load balancer over a single endpoint from failing closed. Set `alwaysEjectOneHost` to permit one ejection
