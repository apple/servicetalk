...
link:https://datatracker.ietf.org/doc/html/rfc7540[HTTP/2] directly for users. The
link:https://github.com/grpc/grpc-java[grpc-java] team and the ServiceTalk team collaborated to bring
link:https://datatracker.ietf.org/doc/html/rfc7540[HTTP/2] support to Netty, and both share the same underlying protocol
implementation. Once you have link:https://datatracker.ietf.org/doc/html/rfc7540[HTTP/2] support the investment to support the
...
To bound memory use, ServiceTalk watches a single decoded inbound gRPC message against a *4 MiB* threshold by default
(matching link:https://github.com/grpc/grpc-java[grpc-java]). This default is *warn-only*: a message whose declared
length exceeds the threshold is still delivered, but a warning is logged (rate-limited to once every five minutes per
client/server) so you can evaluate the limit before enforcing it. The limit applies on the receiving side of every
paradigm: a server watches request messages, a client watches response messages. For a compressed message it watches
the on-wire (still-compressed) length before buffering, and the decoded length after decompression.

Enable enforcement with `GrpcServerBuilder#maxInboundMessageSize(int)` / `GrpcClientBuilder#maxInboundMessageSize(int)`:
a positive value enforces the limit -- a message whose declared length exceeds it is rejected with `RESOURCE_EXHAUSTED`
*before* its payload is buffered -- and `0` disables the limit (negative values are rejected). The default can be
changed globally with the temporary `io.servicetalk.grpc.netty.temporaryDefaultMaxInboundMessageSize` system property
(to be removed in a future release; a positive value enforces at that size, `0` disables, and `-1` keeps warn-only);
an explicit builder call takes precedence.
