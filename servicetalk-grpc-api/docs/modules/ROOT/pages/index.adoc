...
link:https://http2.github.io[HTTP/2] protocol using link:https://netty.io[Netty] similar to
link:https://github.com/grpc/grpc-java[grpc-java] which makes it a relatively lower investment for us to implement the
link:https://github.com/grpc/grpc/blob/master/doc/PROTOCOL-HTTP2.md[gRPC wire protocol].
...
== Overview
ServiceTalk gRPC support provides all the different
...
factory. It manages multiple `Connections` via a
...
The link:{source-root}/servicetalk-client-api/src/main/java/io/servicetalk/client/api/LoadBalancer.java[LoadBalancer] is
consulted for each request to determine which connection should be used. The `LoadBalancer` interface is extensible and
