...
public final class GrpcExceptionMapperServiceFilter implements StreamingHttpServiceFilterFactory {
            ...
            public Single<StreamingHttpResponse> handle(final HttpServiceContext ctx,
                ...
                try {
                    handle = delegate().handle(ctx, request, responseFactory).map(response -> {
                        // FIXME: make this logic optional for users who want to be able to respond for HTTP requests
                    ...
                    return succeeded(convertToGrpcErrorResponse("handle", responseFactory, cause, allocator));
                }
                return handle.onErrorReturn(cause ->
                        convertToGrpcErrorResponse("response", responseFactory, cause, allocator));
            }
        };
    }
    ...
    private static StreamingHttpResponse convertToGrpcErrorResponse(final String what,
        ...
        final StreamingHttpResponse response = newErrorResponse(responseFactory, APPLICATION_GRPC, cause, allocator);
        LOGGER.debug("Mapped a {} error into grpc-status: {}", what, response.headers().get(GRPC_STATUS), cause);
        return response;
    }
        ...
        protected HttpHeaders payloadFailed(final Throwable cause, final HttpHeaders trailers) {
            ...
            if (grpcStatus == null || OK.strValue().contentEquals(grpcStatus)) {
                ...
                if (LOGGER.isDebugEnabled()) {
                    LOGGER.debug("Mapped a payload body error into grpc-status: {}", trailers.get(GRPC_STATUS), cause);
                }
            } else {
                LOGGER.debug("Ignoring a payload body error because another grpc-status: {} is already set",
                        grpcStatus, cause);
            }
