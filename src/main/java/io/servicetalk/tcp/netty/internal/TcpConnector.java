...
public final class TcpConnector<Read, Write> {
    ...
    private ChannelFuture connectWithBootstrap(Object resolvedAddress, EventLoop loop, ChannelHandler handler) {
    ...
    private ChannelFuture initFileDescriptorBasedChannel(EventLoop loop, Channel channel, ChannelHandler handler) {
        for (@SuppressWarnings("rawtypes") Map.Entry<ChannelOption, Object> opt : config.getOptions().entrySet()) {
            //noinspection unchecked
        ...
        // Set the correct ByteBufAllocator based on our BufferAllocator to minimize memory copies.
        channel.config().setAllocator(BufferUtil.getByteBufAllocator(config.getAllocator()));
