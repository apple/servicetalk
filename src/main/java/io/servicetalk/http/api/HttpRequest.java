...
/**
 * HTTP request that provides the content as a stream.
 *
 * <h2>Trailing headers</h2>
 * Trailing headers can be obtained from a request if the type of the content is {@link HttpPayloadChunk}.
...
public interface HttpRequest<T> extends HttpRequestMetaData {
    ...
    /**
     * To modify the {@link #getMessageBody()} of the request and preserving the containing request object.
     *
     ...
     * @return New {@code HttpRequest} with the altered {@link #getMessageBody()}.
     */
    <R> HttpRequest<R> transformMessageBody(Function<Publisher<T>, Publisher<R>> transformer);
}
