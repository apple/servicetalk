...
public final class ExecutionContextRule extends ExternalResource implements ExecutionContext {
    ...
    public static ExecutionContextRule immediate() {
        return new ExecutionContextRule(() -> DEFAULT_ALLOCATOR, NettyIoExecutors::createExecutor, Executors::immediate
        );
    }

    public static ExecutionContextRule cached() {
        return new ExecutionContextRule(() -> DEFAULT_ALLOCATOR, NettyIoExecutors::createExecutor,
    ...
    protected void after() {
        try {
            awaitIndefinitely(ctx.getIoExecutor().closeAsync().mergeDelayError(ctx.getExecutor().closeAsync()));
        } catch (Throwable t) {
