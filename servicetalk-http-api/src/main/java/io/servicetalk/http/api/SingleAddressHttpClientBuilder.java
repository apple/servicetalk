...
public interface SingleAddressHttpClientBuilder<U, R> extends HttpClientBuilder<U, R, ServiceDiscovererEvent<R>> {
     ...
     *     client API and the filters added. Each client API variant (async/blocking streaming/aggregate) requires a
     *     specific execution strategy to avoid blocking the event-loop and filters added via
