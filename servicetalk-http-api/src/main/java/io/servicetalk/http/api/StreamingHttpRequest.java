 ...
 */
public interface StreamingHttpRequest<T> extends HttpRequestMetaData {
    /**
     ...
     * @return New {@link StreamingHttpRequest} with the altered {@link #getPayloadBody()}.
     */
    <R> StreamingHttpRequest<R> transformPayloadBody(Function<Publisher<T>, Publisher<R>> transformer);

    @Override
    StreamingHttpRequest<T> setRawPath(String path);

    @Override
    StreamingHttpRequest<T> setPath(String path);

    @Override
    StreamingHttpRequest<T> setRawQuery(String query);

    @Override
    StreamingHttpRequest<T> setVersion(HttpProtocolVersion version);

    @Override
    StreamingHttpRequest<T> setMethod(HttpRequestMethod method);

    @Override
    StreamingHttpRequest<T> setRequestTarget(String requestTarget);
}
