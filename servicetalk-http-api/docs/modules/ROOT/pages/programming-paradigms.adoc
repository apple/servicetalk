...
consumption, ServiceTalk watches the size of an aggregated body against a *4 MiB* threshold by default. This default
is *warn-only*: an aggregated body that exceeds it is still served/delivered, but a warning is logged (rate-limited to
once every five minutes per client/server) so you can evaluate the limit before enforcing it.
...
therefore counted at its decoded size.

Enable enforcement (or disable the limit) via `HttpServerBuilder#maxAggregatedPayloadSize(int)` /
`SingleAddressHttpClientBuilder#maxAggregatedPayloadSize(int)`: any non-negative value is definitive and always
enforced (pass `0` to disable), so a request exceeding a server's enforced limit is rejected with a
`413 Payload Too Large` and an oversized response surfaces a `PayloadTooLargeException` to the client. To change the
default globally without touching each builder, use the
`io.servicetalk.http.netty.temporaryDefaultMaxAggregatedPayloadSize` system property (a positive value enforces at
that size, `0` disables, and `-1` keeps warn-only). An explicit builder call takes precedence over the property. This
is a temporary property that will be removed in future releases. Bodies consumed purely as a stream are not
affected.
====
