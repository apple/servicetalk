...
`SingleAddressHttpClientBuilder#maxAggregatedPayloadSize(int)` (pass `0` to disable), or globally with the
`io.servicetalk.http.netty.defaultMaxAggregatedPayloadSize` system property. Bodies consumed purely as a stream are
not affected.
====
