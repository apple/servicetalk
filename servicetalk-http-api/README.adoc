...
protocol. This module supports the different <<../README#programming-Paradigms, Programming Paradigms>> for client and
server. Here is a quick start example of the <<../README#blocking-and-aggregated, blocking and aggregated>> paradigm:
...
general they facilitate user code to filter/intercept/modify the request/response processing. Some example usages of
`Filters` include authentication, authorization, logging, metrics, tracing, etc...
...
to your preferred programming paradigm and append it on the
link:src/main/java/io/servicetalk/http/api/HttpServerBuilder.java#L247[HttpServerBuilder].
...
ink:../servicetalk-http-router-jersey[JAX-RS via Jersey]. The general component diagram of a "Router" is as follows:
...
==== Service Blocking and Aggregated
This programming paradigms exposes the request payload body as single aggregated object, and the response payload body
...
==== Service Blocking and Streaming
This programming paradigms exposes the request payload body as an
...
link:src/main/java/io/servicetalk/http/api/HttpOutputStream.java[HttpOutputStream].
...
==== Service Asynchronous and Aggregated
This programming paradigms exposes the request payload body as single aggregated object, and the response payload body
...
==== Service Asynchronous and Streaming
This programming paradigms exposes the request payload body as an
...
This `Client` will only connect to a single address, that is provided upfront at build time. This `Client` is
for use cases where you want to issue requests to a single service (that may have multiple instances).
...
your preferred programming paradigm and append it on the respective
link:src/main/java/io/servicetalk/http/api/HttpClientBuilder.java[HttpClientBuilder].
...
each service address every link:https://tools.ietf.org/html/rfc1035#section-3.2.1[TTL] seconds. The `ServiceDiscoverer`
is not in the request/response path but instead resolves `Connection` addresses "out of band".
...
This programming paradigms expects the request payload body as single aggregated object, and the response payload body
is also a single aggregated object. There are no asynchronous primitives involved.
...
This programming paradigms expects the request payload body as an
link:https://docs.oracle.com/javase/8/docs/api/java/lang/Iterable.html[Iterable] of
link:../servicetalk-buffer-api/src/main/java/io/servicetalk/buffer/api/Buffer.java[Buffer]s and the response payload
body is consumed via an
link:https://docs.oracle.com/javase/8/docs/api/java/lang/Iterable.html[Iterable] of
link:../servicetalk-buffer-api/src/main/java/io/servicetalk/buffer/api/Buffer.java[Buffer]s.
...
This programming paradigms expect the request payload body as single aggregated object, and the response payload body
