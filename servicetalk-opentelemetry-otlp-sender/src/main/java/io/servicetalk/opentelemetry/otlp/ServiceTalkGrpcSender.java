...
final class ServiceTalkGrpcSender implements GrpcSender {
    ...
    ServiceTalkGrpcSender(StreamingHttpClient httpClient,
        ...
        // OTel supplies the method name without a leading slash (e.g.
        // "opentelemetry.proto.collector.trace.v1.TraceService/Export"), but a gRPC :path must be
        // absolute. Prepend "/" so strict gRPC servers (e.g. grpc-java) route the call rather than 404.
        String httpPath = fullMethodName.startsWith("/") ? fullMethodName : "/" + fullMethodName;
        MethodDescriptor<byte[], byte[]> md = MethodDescriptors.newMethodDescriptor(
                httpPath, "send", false, false,
