...
public interface SingleFirstStep<T> extends SingleLastStep<T> {
     ...
     */
    SingleLastStep<T> expectCancellable(Consumer<? super Cancellable> predicate);

    /**
     * Expect no {@link Subscriber#onSuccess(Object)} or {@link Subscriber#onError(Throwable)} signals in
     * {@code duration} time.
     * @param duration The amount of time to wait for a terminal signal.
     * @return An object which allows to verify all expectations.
     */
    SingleFirstStep<T> expectNoTerminal(Duration duration);
}
