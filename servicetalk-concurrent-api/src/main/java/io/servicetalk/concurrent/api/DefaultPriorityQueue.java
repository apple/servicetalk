...
import static io.servicetalk.concurrent.api.DefaultPriorityQueue.Node.INDEX_NOT_IN_QUEUE;
import static java.util.Objects.requireNonNull;

/**
 * A priority queue which uses natural ordering of elements. Elements are also required to be of type
 * {@link Node} for the purpose of maintaining the index in the priority queue.
 * @param <T> The object that is maintained in the queue.
 */
final class DefaultPriorityQueue<T extends DefaultPriorityQueue.Node> extends AbstractQueue<T> {
    ...
    /**
     * Provides methods for {@link DefaultPriorityQueue} to maintain internal state. These methods should generally not
     * be used outside the scope of {@link DefaultPriorityQueue}.
     */
    interface Node {
        /**
        ...
        /**
         * Get the last value set by {@link #priorityQueueIndex(DefaultPriorityQueue, int)} for the value corresponding
         * to {@code queue}.
         * <p>
    ...
    DefaultPriorityQueue(Comparator<T> comparator, int initialSize) {
        ...
        queue = (T[]) (initialSize != 0 ? new Node[initialSize] : EMPTY_ARRAY);
    }
    ...
    boolean priorityChanged(T node) {
        int i = node.priorityQueueIndex(this);
        if (!contains(node, i)) {
            return false;
        }
