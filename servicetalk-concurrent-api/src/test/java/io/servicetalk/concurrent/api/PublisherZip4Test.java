...
final class PublisherZip4Test {
    ...
    void allComplete(boolean inOrderCompletion, boolean onError) throws InterruptedException {
        toSource(zip(first, second, third, fourth, PublisherZip4Test::combine)).subscribe(subscriber);
        ...
        assertThat(subscriber.takeOnNext(), equalTo(combine(i, d, s, b)));
    ...
    void maxOutstandingDemand(boolean inOrderCompletion, boolean onError) throws InterruptedException {
        toSource(zip(first, second, third, fourth, PublisherZip4Test::combine, 1)).subscribe(subscriber);
    ...
    void reentry(int begin, int end) throws InterruptedException {
        ...
        toSource(zip(fromSource(new ReentryPublisher(begin, end)),
                ...
                fromSource(new ReentryPublisher(begin, end)),
                fromSource(new ReentryPublisher(begin, end)),
                PublisherZip4Test::combineReentry, 1))
        ...
        for (int i = begin; i < end; ++i) {
            signal = signals.take();
            assertThat(signal, equalTo(combineReentry(i, i, i, i)));
        }
        signal = signals.take();
    ...
    void delayError(boolean inOrderCompletion, boolean onError) throws InterruptedException {
        toSource(zipDelayError(first, second, third, fourth, PublisherZip4Test::combine)).subscribe(subscriber);
        subscriber.awaitSubscription().request(1);
        int i = 3;
        double d = 10.23;
        short s = 88;
    ...
    private static String combineReentry(int i, int d, int s, int b) {
        return "first: " + i + " second: " + d + " third: " + s + " fourth:" + b;
    }
