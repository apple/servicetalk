...
class H2ToStH1UtilsTest {
    ...
    /**
     * An octet is permitted in a {@code :path} iff it is visible US-ASCII (VCHAR, 0x21-0x7e); see
     * {@link H2ToStH1Utils#invalidPathReason(CharSequence)} for the RFC derivation. Checking all three positions
     * matters because the generic field rules of RFC 9113 8.2.1 prohibit SP/HTAB only at the edges, whereas the 8.3.1
     * {@code :path} production admits neither anywhere. This one sweep is the whole character contract.
     */
    ...
    void pathOctetContract(int octet) {
        ...
        for (final String path : asList(c + "/ab", "/a" + c + "b", "/ab" + c)) {
            ...
            // AsciiString is the type HPACK produces on the inbound path; charAt() masks to 0x00-0xff so the same
            // scan applies, but assert it so the ingress type is covered.
    ...
    /**
     * obs-text is rejected even though RFC 9110 5.5 permits it in a generic field value, because RFC 9113 8.3.1
     * defines {@code :path} as the RFC 3986 {@code absolute-path [ "?" query ]} production, which is US-ASCII only:
     * a non-ASCII character has to be percent-encoded by the producer. Named explicitly so that editing
     * {@link #pathOctetContract(int)}'s single predicate cannot silently re-permit the range.
     */
    @ParameterizedTest(name = "{displayName} [{index}] octet={0}")
    ...
    /**
     * Regression guard against narrowing the scan to a single octet (a table indexed by {@code c & 0xff}, or a
     * {@code (byte) c} cast): each of these chars has a low byte that is a legal VCHAR, so narrowing would accept it.
     */
    ...
    void invalidPathReasonMessages() {
        // Absent, empty and illegal-character all report through the one method.
        ...
        // Same "expected" wording as HttpObjectDecoder's request-target check.
