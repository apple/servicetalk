...
import static io.servicetalk.http.api.BlockingStreamingHttpRequests.newRequest;
import static io.servicetalk.http.api.HttpRequestMethods.GET;
import static java.nio.charset.StandardCharsets.US_ASCII;

public final class HelloWorldBlockingStreamingClient {
    private static final Logger LOGGER = LoggerFactory.getLogger(HelloWorldBlockingStreamingClient.class);

    public static void main(String[] args) throws Exception {
        // Collection of all resources in this test that can be closed together at the end.
        try (BlockingStreamingHttpClient client = HttpClients
                .forSingleAddress("localhost", 8080)
                .buildStreaming().asBlockingStreamingClient()) {

            // Create a request, send the request, and wait for the response.
            BlockingStreamingHttpResponse<HttpPayloadChunk> response = client.request(newRequest(GET, "/sayHello"));
